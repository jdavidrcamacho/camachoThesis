...
time,rv,rverr,rhk,rhkerr= np.loadtxt("sunBinned_Dumusque.txt", skiprows = 1, 
                                    unpack = True,
                                    usecols = (0,1,2,3,4))
...
axs['predictive 1'].plot(tstar, a[0].T, '-g')
axs['predictive 1'].fill_between(tstar,  bmax1.T, bmin1.T, color="green", alpha=0.25)
axs['predictive 1'].tick_params(axis='both', which='both', labelbottom=False)
axs['predictive 2'].set(xlabel='Time (BJD - 2450000)', ylabel='$\log R^{\'}_{hk}$')
axs['predictive 2'].errorbar(time, val2, val2err, fmt= '.k',  alpha=0.5)
axs['predictive 2'].plot(tstar, a[1].T, '-g')
axs['predictive 2'].fill_between(tstar,  bmax2.T, bmin2.T, color="green", alpha=0.25)

axs['weight 1'].set(xlabel='', ylabel='RV weight (m/s)')
axs['weight 1'].plot(tstar, bb[1,0].T, '-g')
axs['weight 1'].tick_params(axis='both', which='both', labelbottom=False)
axs['mean 1'].set(xlabel='', ylabel='RV mean (m/s)')
axs['mean 1'].plot(tstar, means[0](tstar), '-', color='green')
axs['mean 1'].tick_params(axis='both', which='both', labelbottom=False)

axs['node'].set(xlabel='', ylabel='Node')
axs['node'].plot(tstar, bb[0,0].T, '-g')
axs['node'].tick_params(axis='both', which='both', labelbottom=False)
axs['weight 2'].set(xlabel='',  ylabel='$\log R^{\'}_{hk}$ weight')
axs['weight 2'].plot(tstar, bb[1,1].T, '-g')
axs['weight 2'].tick_params(axis='both', which='both', labelbottom=False)
axs['mean 2'].set(xlabel='Time (BJD - 2450000)', ylabel='$\log R^{\'}_{hk}$ mean')
axs['mean 2'].plot(tstar, means[1](tstar), '-', color='green')
axs['mean 2'].tick_params(axis='both', which='both', labelbottom=True)

plt.tight_layout(pad=0.1, h_pad=0.25, w_pad=0.5)

fig.savefig('fullPlots_RVRHK.pdf', bbox_inches='tight')
# plt.close('all')
