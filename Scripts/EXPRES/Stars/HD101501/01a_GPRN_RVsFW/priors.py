import numpy as np
data = np.loadtxt("101501_activity.csv",delimiter=',', skiprows=1, usecols=(1,4,5,11,12, 13))
time = data[:,0].T
#rv
val1, val1err = data[:,1].T, data[:,2].T
#bis
val2, val2err = data[:,5].T, 2*val1err
#fwhm
...
#node function
neta1 = stats.loguniform(0.1, 2*np.max([val1.ptp(), val3.ptp()]))
...
def priors():
    return np.array([neta1, neta2, neta3, neta4, 
                     weta1_1, weta2_1, weta1_3, weta2_3, 
                     slope1, offset1,  slope3, offset3, 
                     jitt1, jitt3])
