...
def from_prior():
    ...
    return np.array([prior[0].rvs(), new_neta2, prior[2].rvs(), prior[3].rvs(),
...
def elboCalc(thetas):
    ...
    if np.isinf(logcdfSEeta21 or logcdfSEeta22):
        return -np.inf
...
def negativePosterior(thetas):
    return -elboCalc(thetas)

x0 = from_prior()
ndim = x0.size 
print('\nMinimizing...')
res = minimize(negativePosterior, x0, method='Nelder-Mead', 
               options={'disp': False,'maxiter': 1000})


def minimizedPrior():
    n1,n2,n3,n4, w11,w21, w12,w22, s1,off1, s2,off2, j1,j2 = res.x
    values = []
    for i,j in enumerate(res.x):
        values.append(np.random.normal(j, 0.1*j, 1))
...
p0=[minimizedPrior() for i in range(nwalkers)]
print("\nRunning...")
