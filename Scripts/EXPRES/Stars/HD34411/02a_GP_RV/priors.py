import numpy as np
##### Setting priors #####
from scipy import stats
from loguniform import ModifiedLogUniform
stats.loguniform = stats.reciprocal

def priors(filename, RV=True, FWHM=False):
    data = np.loadtxt(filename, delimiter=',', skiprows=1, usecols=(1,4,5,11,12))
    time = data[:,0].T
    val1, val1err = data[:,1].T, data[:,2].T
    val2, val2err = data[:,3].T, 2*val1err
    
    #gp
    neta1MIN, neta1MAX = [], []
    if RV:
        neta1MIN.append(0.1)
        neta1MAX.append(2*val1.ptp())
    if FWHM:
        neta1MIN.append(0.1)
        neta1MAX.append(2*val2.ptp())
    neta1 = ModifiedLogUniform(0.1, 2*np.max(neta1MAX))
    neta2 = stats.loguniform(0.1*np.ediff1d(time).mean(), 10*time.ptp())
    neta3 = stats.uniform(5, 50 -5)
    neta4 = stats.loguniform(0.1, 5)
    priorArray = [neta1, neta2, neta3, neta4]
    
    #Mean function
    slope1 = stats.norm(0, 1)
    offset1 = stats.uniform(val1.min(), val1.max() -val1.min())
    slope2 = stats.norm(0, 1)
    offset2 = stats.uniform(val2.min(), val2.max() -val2.min())
    if RV:
        priorArray.append(slope1)
        priorArray.append(offset1)
    if FWHM:
        priorArray.append(slope2)
        priorArray.append(offset2)
    
    #jitters
    jitt1 = ModifiedLogUniform(0.1, 2*val1.ptp())
    jitt2 = ModifiedLogUniform(0.1, 2*val2.ptp())
    if RV:
        priorArray.append(slope1)
        priorArray.append(offset1)
    if FWHM:
        priorArray.append(slope2)
        priorArray.append(offset2)

    return np.array(priorArray)
